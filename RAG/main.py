...
Assistant: """
    ...
    # If conversation_id is provided, load the chat history
    if conversation_id:
        cursor.execute("SELECT chat_history FROM conversations WHERE id = ? AND project_id = ?", conversation_id, project_id)
    ...
    result = qa_chain({"human_input": question, "question": question})
    answer = result['answer']
    
    # Save or update the conversation
    if conversation_id:
        chat_history = memory.chat_memory.messages
        chat_history_json = json.dumps([{"human": m.content, "ai": chat_history[i+1].content} 
                                        for i, m in enumerate(chat_history) if i % 2 == 0])
        cursor.execute("UPDATE conversations SET chat_history = ? WHERE id = ?", chat_history_json, conversation_id)
    else:
        chat_history = [{"human": question, "ai": answer}]
        chat_history_json = json.dumps(chat_history)
        cursor.execute("INSERT INTO conversations (project_id, chat_history) VALUES (?, ?)", project_id, chat_history_json)
        conversation_id = cursor.execute("SELECT @@IDENTITY").fetchone()[0]

    conn.commit()

    # Get the sources of the answer
...
def get_conversation(conversation_id):
    cursor.execute("SELECT chat_history FROM conversations WHERE id = ?", conversation_id)
    result = cursor.fetchone()
    if result:
        chat_history = json.loads(result.chat_history)
